...
class AssertDefaultSpecId(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr
    default_spec_id: StrictInt = Field(alias="default-spec-id")
    __properties: ClassVar[List[str]] = ["type", "default-spec-id"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['assert-default-spec-id']):
            raise ValueError("must be one of enum values ('assert-default-spec-id')")
        return value
