...
class SetDefaultSortOrderUpdate(BaseUpdate):
    ...
    """ # noqa: E501
    action: StrictStr
    sort_order_id: StrictInt = Field(description="Sort order ID to set as the default, or -1 to set last added sort order", alias="sort-order-id")
    __properties: ClassVar[List[str]] = ["action"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['set-default-sort-order']):
            raise ValueError("must be one of enum values ('set-default-sort-order')")
        return value
